...
filterByChanged() {
  if (this.filterBy == 'motif') {
    ...
    this.datas.sort((a, b) => a.codeproduit.localeCompare(b.codeproduit));
  ...
  try {
    if (this.searchText) {
      this.datas = await this.produitDtos.filter(produit => {
        ...
        || produit.designation.toLowerCase().includes(this.searchText.toLowerCase())
        || produit.codeproduit.toLowerCase().includes(this.searchText.toLowerCase());
      });
    } else {
