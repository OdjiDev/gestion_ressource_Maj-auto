              ...
              <tr>
                @for (col of columns; track col.key) {
                  <td [style.text-align]="col.align || 'left'">
                    @switch (col.type) {
                      @case ('badge') {
                        <span class="badge">{{ getCellValue(row, col) }}</span>
                      }
                      @case ('date') {
                        {{ getCellValue(row, col) | date: 'dd/MM/yyyy' }}
                      }
                      @case ('currency') {
                        {{ getCellValue(row, col) | number: '1.0-0' }} FCFA
                      }
                      @default {
                ...
                @if (showActions) {
                  <td class="actions-col">
                    @if (showEdit) {
                      <button class="btn-icon btn-edit" (click)="edit.emit(row)" title="Modifier">
                        ✏️
                      </button>
                    }
                    @if (showDelete) {
...
export class DataTableComponent<T = any> {
  ...
  trackBy(row: T): any {
    return (row as any)[this.trackByField];
  }
}
