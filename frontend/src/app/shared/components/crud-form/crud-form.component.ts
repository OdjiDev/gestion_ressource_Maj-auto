    ...
    <form [formGroup]="form" (ngSubmit)="onSubmit()" class="crud-form">
        ...
        @for (field of config.fields; track field.name) {
          <div class="form-field" [class.col-span-2]="field.colSpan === 2">
            ...
            @switch (field.type) {
              ...
              @case ('checkbox') {
                <label class="checkbox-label"
                  ><input type="checkbox" [formControlName]="field.name" /><span>{{
            ...
            @if (field.type !== 'checkbox') {
              <app-field-error-display [control]="getControl(field.name)" [label]="field.label" />
            }
          </div>
        }
      </div>
      <div class="form-actions">
        @if (config.showCancel !== false) {
          <button type="button" class="btn btn-cancel" (click)="cancel.emit()">
            {{ config.cancelLabel || 'Annuler' }}
          </button>
        }
        <button type="submit" class="btn btn-primary" [disabled]="form.invalid || submitting">
          {{ config.submitLabel || 'Enregistrer' }}
        </button>
      </div>
    </form>
  `,
...
export class CrudFormComponent implements OnInit {
  ...
  private buildForm(): void {
    ...
    for (const field of this.config.fields) {
      ...
      const initialValue =
        this.initialData?.[field.name] ?? field.defaultValue ?? this.getDefaultValue(field.type);
